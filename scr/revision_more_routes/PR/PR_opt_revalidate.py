...
db_opt_result = client.cota_re_pr_opt
db_opt_finalization = client.cota_re_pr_opt
...
def validate_stop_time(single_date):
    ...
    col_opt_result = db_opt_finalization["FIN_" + today_date + "_" + str(int(the_route_id))] # Finalized
    col_smart_transit = db_opt_result["REV_" + today_date+ "_" + str(int(the_route_id))] # Revalidation
    ...
    rs_all_trips = list(db_new_trips.find({"service_id": str(
        service_id), "route_id":  the_route_id}))  # Control the size of sampling
    count = 0
