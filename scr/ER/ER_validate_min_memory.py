...
def validate_er(single_date, memory):
    ...
    if start_date_p - date(2018, 2, 1) <0:
        return False
    col_er = db_er_opt["er_min_" + str(memory) + "_" + start_date_p.strftime("%Y%m%d")]
...
if __name__ == "__main__":
    ...
    for memory in range(1, 10):
        ...
        output = []
        output = pool.starmap(validate_er, zip(date_range,[memory]*len(date_range)))
        pool.close()
        pool.join()
