...
db_er_opt = client.cota_re_er_cal
db_er_val = client.cota_re_er_val

def validate_er(single_date, buffer):
    memory = 6
    start_date_p = single_date - timedelta(days=memory)
    if (start_date_p - date(2018, 5, 7)).total_seconds() < 0:
        return False
    col_er = db_er_opt[start_date_p.strftime("%Y%m%d") + "_" +str(buffer)]
    ...
    col_er_val = db_er_val[today_date + "_" + str(buffer)]
    ...
    for each_record in rl_real_time:
        ...
        if query_rl == None:
            continue
        each_record["time_er_arr"] = int(query_rl["time"] + today_seconds) - buffer
