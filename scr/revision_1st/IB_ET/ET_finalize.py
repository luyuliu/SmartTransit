...
db_opt_fin = client.cota_re_er_val
...
def calculate_parameters(end_date_p):
    ...
    col_opt_fin = db_opt_fin["FIN_" + end_date_p.strftime("%Y%m%d") ]
    ...
    for single_date in transfer_tools.daterange(start_date, end_date_p):
        ...
        col_opt_result = db_opt_result["OPT_" + today_date]
        ...
        for each_record in rl_opt_result:
            ...
            try:
                ...
                records_dic[trip_id][stop_id]["sum_buff"] = each_record["optima_buffer"]
                records_dic[trip_id][stop_id]["max_buff"] = each_record["optima_buffer"]
                records_dic[trip_id][stop_id]["date_count"] = 1
            else:
                records_dic[trip_id][stop_id]["sum_buff"] += each_record["optima_buffer"]
                records_dic[trip_id][stop_id]["max_buff"] = max(each_record["optima_buffer"], records_dic[trip_id][stop_id]["max_buff"])
                records_dic[trip_id][stop_id]["date_count"] += 1
