...
db_smart_transit = client.cota_re_gt_cal
...
def analyze_transfer(criteria, each_date):
    buffer = 0
    ...
    if today_weekday < 5:
        ...
        service_id = 3
    db_today_smart_transit = db_smart_transit[today_date+"_"+str(criteria)]
    ...
    for single_trip in rs_all_trips:
        ...
        for single_stop_time in rs_all_stops:
            ...
            for time_walking in range(walking_time_limit):
                ...
                # Time rAlt: Time for smart transit user's actual onboard time with running

                line["time_ralt_" + str(time_walking)] = 9999999999

                for i_real_time in alt_times_list:
                    # relaxed
                    if line["time_ralt_" + str(time_walking)] > i_real_time and i_real_time >= line["time_smart_" + str(time_walking)] - criteria:
                        line["time_ralt_" + str(time_walking)] = i_real_time

                if line["time_ralt_" + str(time_walking)] == 9999999999:
                    # there's no an alternative trip.
                    line["time_ralt_" + str(time_walking)] = "critical_trip"
...
if __name__ == '__main__':
    ...
    for each_date in transfer_tools.daterange(start_date, end_date):
        ...
        pool = multiprocessing.Pool(processes=35)
        output = []
        output = pool.starmap(analyze_transfer, zip(criterias,[each_date]*len(criterias)))
        pool.close()
        pool.join()
