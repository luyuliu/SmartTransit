...
def analyze_transfer(single_date):
    ...
    col_pr_opt_result = db_smart_transit["OPT_MIN_" + today_date + "_" + str(the_route_id)] # only use the date before the training day; And use risk averse strategy
    pre_count = col_pr_opt_result.estimated_document_count()
    count = db_smart_transit[today_date+"_0_" + str(the_route_id)].estimated_document_count()
    ...
    for each_buffer in insurance_buffers:
        db_today_smart_transit = db_smart_transit[today_date+"_"+str(each_buffer) + "_" + str(the_route_id)]
